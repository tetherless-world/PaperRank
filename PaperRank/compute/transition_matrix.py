...
class MarkovTransitionMatrix:
    ...
    def construct(self) -> sparse.csr_matrix:
        ...
        print('Initializing {0}x{0} transition matrix'.format(self.N))
    ...
    def __buildUnadjustedMatrix(self, M: sparse.dok_matrix) \
        ...
        print('Building unadjusted transition matrix')
        ...
        for i in range(self.N):
            ...
            for inbound in inbound_list:
                ...
                #if j == False:
                #    logging.warn('Inbound citation {0} for paper {1} not indexed'.format(inbound, paper_id))
                #    continue
            ...
            # Log progress
            print("--- %s seconds elapsed since start ---" % (time.time() - start_time))
            last_check = logLoopProgress(i, last_check, self.N,
                                         'Unadjusted transition matrix')

        print('Built unadjusted Markov transition matrix with {0} elements'.format(M.nnz))
    ...
    def __adjustTransitionMatrix(self, M: sparse.csc_matrix) \
        ...
        print('Building adjusted transition matrix')
